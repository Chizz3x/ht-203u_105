package com.hti.xtherm.seekbar;
...
public class RangeSeekBar extends View {
    ...
    private void initAttrs(AttributeSet attributeSet) {
        try {
            ...
            this.tickMarkInRangeTextColor = typedArrayObtainStyledAttributes.getColor(R.styleable.RangeSeekBar_rsb_tick_mark_in_range_text_color, this.progressColor);
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (mode == MeasureSpec.EXACTLY) {
            iMakeMeasureSpec = MeasureSpec.makeMeasureSpec(size, MeasureSpec.EXACTLY);
        } else if (mode == Integer.MIN_VALUE && (getParent() instanceof ViewGroup) && size == -1) {
            iMakeMeasureSpec = MeasureSpec.makeMeasureSpec(((ViewGroup) getParent()).getMeasuredHeight(), MeasureSpec.AT_MOST);
        } else {
            if (this.gravity == 2) {
                if (this.tickMarkTextArray != null && this.tickMarkLayoutGravity == 1) {
            ...
            iMakeMeasureSpec = MeasureSpec.makeMeasureSpec((int) rawHeight, MeasureSpec.EXACTLY);
        }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 1) {
            if (verifyStepsMode() && this.stepsAutoBonding) {
                ...
                this.currTouchSB.slide(new BigDecimal(fCalculateCurrentSeekBarPercent / this.steps).setScale(0, RoundingMode.HALF_UP).intValue() * (1.0f / this.steps));
            }
            if (this.seekBarMode == 2) {
    ...
    public void setProgress(float f, float f2) {
        ...
        if (f3 < f4) {
            if (fMin - this.minProgress > this.maxProgress - fMax) {
                fMin = fMax - f4;
            } else {
                fMax = fMin + f4;
            }
